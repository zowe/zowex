...
auto-deploy path ships only `zowex` (to `~/.zowe-server`); marking a file
APF-authorized is a system-programmer action, and installing `zoweax` is the
...
Neither the `zoweax` CLI nor the `consoleCommand` RPC requests an authority
...
`zoweax` — grant command authority with OPERCMDS profiles instead. (Programs
...
> minimum shape of what `zoweax` needs; they are not a substitute for your
...
the logonid. An SSH user therefore needs both records before `zoweax` can run:
