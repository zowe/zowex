...
- Updated the checkIfOutdated() utility to compare version numbers with semantic versioning rather than comparing checksums of the `zowex` program. [#TODOPRNUMBER](PR link)
- Removed the checksums.asc file from `zowex` server pax files. [#TODOPRNUMBER](PR link)
