...
- `c`: Added a declared minimum supported z/OS level to the build, `ZosMinLevel=zosv2r5` in `native/c/toolchain.mk`, passed to the compiler as `-mzos-target`. The floor is enforced at bind time: CI now binds against Language Environment side decks captured from a system at that release, so a libc++ symbol the release does not export fails the build instead of failing at load time with `CEE3561S`. `make runtime-imports` (`npm run z:imports`) reports the symbols the binary imports, and `npm run lint:compat` rejects string-keyed hash containers at the source level. [#871](https://github.com/zowe/zowex/issues/871)
