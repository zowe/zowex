...
void parser_tests()
{
  describe("parser tests", []() -> void
           { describe("dynamic keyword arguments", []() -> void
             ...
             describe("post-command hooks", []() -> void
                      {
             it("executes hooks after command handler", []() {
               g_handler_called = false;
               bool hook_called = false;
               bool hook_is_help = true;
               int received_exit_code = -1;

               ArgumentParser arg_parser("prog", "post hook sample");
               arg_parser.add_post_command_hook([&](const Command &cmd, bool is_help_request, int exit_code) -> bool {
                 hook_called = true;
                 hook_is_help = is_help_request;
               ...
               Expect(hook_called).ToBe(true);
               Expect(hook_is_help).ToBe(false);
               Expect(received_exit_code).ToBe(7); // sample_handler returns 7
             });
             });
