...
- Interactive `zowex` CLI passthroughs (`zx tool`, `zx console`, `zx system parmlib|subsystems|symbol`, `zx ds copy`) work the same — they run over ssh, not the session pipe.